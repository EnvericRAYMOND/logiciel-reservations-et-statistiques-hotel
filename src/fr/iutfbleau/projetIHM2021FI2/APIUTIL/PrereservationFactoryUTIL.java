 ...
 *
 */
public class PrereservationFactoryUTIL implements PrereservationFactory{
    ...
    //  * Constructeur
    //  */
     public PrereservationFactoryUTIL(){
        try{
            ...
            PreparedStatement pst = cnx.prepareStatement("SELECT id,prenom,nom FROM Client");
            ResultSet rs = pst.executeQuery();
            int i=0;
            while(rs.next()){
                i++;

                clients[i] =  new ClientNP(Integer.parseInt(rs.getString(1)),rs.getString(2),rs.getString(3));
            }
            pst = cnx.prepareStatement("SELECT reference,debut,nuits,categorie,idClient FROM Reservations");
            rs = pst.executeQuery();
	        DateTimeFormatter sdf = DateTimeFormatter.ofPattern("dd/MM/yyyy");
	        LocalDate date=null;
            int cli=0;
            TypeChambre c=TypeChambre.UNLS;
            while(rs.next()){
		      try{
		      date=LocalDate.parse(rs.getString(2),sdf);  
		      }catch(DateTimeParseException g){}
		      for (int y=1;y<=100;y++){
		          if(clients[y].getId()==Integer.parseInt(rs.getString(5))){cli=y;}
		      }
	      if(rs.getString(4).equals("UNLS")){c=TypeChambre.UNLS;}
              if(rs.getString(4).equals("UNLD")){c=TypeChambre.UNLD;}
              if(rs.getString(4).equals("DEUXLS")){c=TypeChambre.DEUXLS;}
              //System.out.println("preres: " + rs.getString(1));
              ajoutePrereservation(rs.getString(1), date ,Integer.parseInt(rs.getString(3)),c,clients[cli]);
            }
            cnx.close();
        }
